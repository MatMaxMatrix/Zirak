...
def restore_backup(file_path):
    ...
    if os.path.exists(backup_path):
        ...
        console.print(f"[bold red]Error: Backup file {backup_path} not found[/bold red]")
        return False
...
def patch_conversation_workflow(file_path):
    ...
    try:
        ...
        if "# PATCHED FOR BETTER WORKFLOW MONITORING" in content:
            console.print("[bold yellow]File already appears to be patched. Skipping.[/bold yellow]")
            return True
    ...
    def send_message_to_group_chat"""
        ...
        for pattern, replacement in patches:
            if pattern in content:
                ...
                console.print(f"[bold green]Applied patch: {pattern[:30]}...[/bold green]")
            else:
                console.print(f"[bold yellow]Pattern not found: {pattern[:30]}...[/bold yellow]")
...
"""
    ...
    def send("""
        ...
        console.print("[bold green]Successfully patched the agent_manager.py file.[/bold green]")
...
def main():
    parser = argparse.ArgumentParser(description="Patch Zirak API for better workflow monitoring")
    parser.add_argument("--workflow-path", default="Agents/conversation_workflow.py", help="Path to conversation_workflow.py")
    parser.add_argument("--agent-manager-path", default="Agents/agent_manager.py", help="Path to agent_manager.py")
    parser.add_argument("--api-path", default="api.py", help="Path to api.py")
    parser.add_argument("--restore", action="store_true", help="Restore from backup instead of patching")
    ...
    if args.restore:
        ...
        if workflow_restored and agent_manager_restored and api_restored:
            console.print("[bold green]All files successfully restored from backup.[/bold green]")
        else:
            console.print("[bold yellow]Some files could not be restored. See errors above.[/bold yellow]")
    else:
        ...
        if workflow_patched and agent_manager_patched and api_patched:
            ...
            console.print("[bold yellow]Some files could not be patched. See errors above.[/bold yellow]")
