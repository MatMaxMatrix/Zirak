...
def state_transition(last_speaker, groupchat):
    ...
    try:
        ...
        console.print(
            f"[bold cyan]State transition from: {last_speaker.name}[/bold cyan]"
        )

        # Initial message handling
        if len(messages) <= 1:
            console.print(
                "[bold cyan]Initial message - selecting initiating agent[/bold cyan]"
            )
        ...
        if last_speaker is UserProxy_agent:
            console.print(
                "[bold cyan]User proxy agent finished - proceeding to LLM Agent[/bold cyan]"
            )
            return LLM_agent
        ...
        if last_speaker is initiating_agent:
            ...
            console.print(
                f"[bold cyan]Initiating agent finished, requires_clarification: {requires_clarification}[/bold cyan]"
            )
        ...
        if last_speaker is CriticalAnalysisAgent:
            try:
                ...
                console.print(
                    f"[bold cyan]Critical Analysis finished, requires_clarification: {requires_clarification}[/bold cyan]"
                )

                if requires_clarification and len(clarifications) < 200:
                    console.print(
                        "[bold cyan]Needs clarification - returning to initiating agent[/bold cyan]"
                    )
                    return initiating_agent
                else:
                    console.print(
                        "[bold cyan]No clarification needed - proceeding to Query Agent[/bold cyan]"
                    )
                    return Query_Agent
            except json.JSONDecodeError as e:
                console.print(
                    f"[bold red]JSON decode error in Critical Analysis transition: {str(e)}[/bold red]"
                )
                return Query_Agent
        ...
        # Query Agent transitions
            ...
            console.print(
                "[bold cyan]Query Agent finished - proceeding to Step Generator[/bold cyan]"
            )
            return Step_agent
        ...
        # Step Generator transitions
            ...
            try:
                ...
                step_response = json.loads(
                    Step_agent.last_message().get("content", "{}")
                )
                context["steps"] = step_response.get("steps", {})
                console.print(
                    f"[bold cyan]Step Generator finished - found {len(context['steps'])} steps[/bold cyan]"
                )
                ...
                console.print(
                    f"[bold red]Error parsing Step Generator response: {str(e)}[/bold red]"
                )
                return Step_agent
        ...
        # LLM Agent transitions
            ...
            console.print(
                "[bold cyan]All steps completed - returning to user proxy agent[/bold cyan]"
            )
        ...
        console.print(
            "[bold cyan]Default transition - returning to initiating agent[/bold cyan]"
        )
        return initiating_agent
