...
def workflow_update(data):
    ...
    elif "context set for agent" in message.lower() or "starting conversation" in message.lower():
        # Setup messages
        if verbose_mode:
            console.print(Panel(
...
def monitor_direct_api_events(base_url, conversation_id=None, interval=1):
    ...
    if conversation_id:
        workflow_url = f"{base_url}{ENDPOINTS['workflow']}?conversation_id={conversation_id}"
        try:
            response = requests.get(workflow_url, timeout=5)
            if response.status_code == 200:
                data = response.json()
                console.print(f"[bold green]Retrieved workflow status: {len(data.get('events', []))} events[/bold green]")
            ...
            console.print(f"[bold yellow]Could not retrieve workflow status: {str(e)}[/bold yellow]")
        ...
        if "processing message from" in message.lower():
            ...
            if len(parts) > 1:
                ...
                if key in agent_messages:
                    content = agent_messages[key][:100] + "..." if len(agent_messages[key]) > 100 else agent_messages[key]
...
def display_conversation_tree():
    ...
    for step in workflow_steps:
        message = step.get('message', '')
        agent = step.get('agent', 'Unknown')
        formatted_time = datetime.fromisoformat(step.get('timestamp', datetime.now().isoformat())).strftime('%H:%M:%S')
...
def display_questions_and_answers():
    ...
    for i, qa in enumerate(qa_pairs, 1):
        ...
        console.print(f"[bold {agent_color}]Q{i} from {agent_name}:[/bold {agent_color}]")
...
def poll_for_updates(base_url, max_time=60, inactivity_timeout=10, poll_interval=0.5):
    ...
    console.print(f"[bold cyan]Collecting agent communications for up to {max_time} seconds...[/bold cyan]")
    ...
    with Progress(
        SpinnerColumn(),
        TextColumn("[bold blue]Monitoring agent communications...[/bold blue] (Press Ctrl+C to stop manually)"),
        console=console
    ) as progress:
...
def monitor_existing_conversation(base_url, conversation_id, max_time=60):
    ...
    try:
        ...
        if not success:
            console.print("[yellow]Could not retrieve existing workflow events. Will monitor for new events.[/yellow]")
        ...
        console.print(f"[bold green]Full conversation data saved to {output_file}[/bold green]")
...
def main():
    parser = argparse.ArgumentParser(description="Monitor agent workflow messages in real-time")
    parser.add_argument("query", nargs="?", help="Query to send to the API (leave empty to monitor existing conversation)")
    ...
    parser.add_argument("--max-time", type=int, default=60, help="Maximum time to monitor (seconds)")
    ...
    if args.conversation_id:
        ...
        console.print("[bold red]Error: Must provide either a query or a conversation ID to monitor.[/bold red]")
        parser.print_help()
        sys.exit(1)
