            ...
            if edit_type == "full":
                ...
                if os.path.exists(file_path):  # ← Moved existence check here
                    ...
                    if start_line is not None and end_line is not None:
                        updated_content = self._edit_by_lines(
                            lines, start_line, end_line, new_content
                        )
                    elif search_pattern and replacement_text:
                        updated_content = self._find_and_replace(
                            original_content, search_pattern, replacement_text
                        )
                    else:
                        raise ValueError("Invalid partial edit parameters")
                else:
                    raise ValueError(
                        "File must exist for partial edits"
                    )  # ← New error message
