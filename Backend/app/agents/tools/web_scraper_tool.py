    ...
    def execute(self, **kwargs) -> str:
        ...
        try:
            ...
            main_container = (
                ...
                or soup.find(attrs={"id": re.compile(r"(main|content|article)", re.I)})
                or soup.find(
                    attrs={"class": re.compile(r"(main|content|article)", re.I)}
                )
            )
            ...
            for elem in main_container.find_all(
                attrs={"class": re.compile(r"(sidebar|nav|menu|ad|advert)", re.I)}
            ):
                elem.decompose()
            for elem in main_container.find_all(
                attrs={"id": re.compile(r"(sidebar|nav|menu|ad|advert)", re.I)}
            ):
                elem.decompose()
