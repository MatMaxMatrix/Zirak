...
def check_server(url="http://localhost:5001"):
    ...
    for endpoint in endpoints:
        ...
        try:
            ...
            if response.status_code < 400:  # Any successful response
                console.print(
                    f"[bold green]✓ Server is running! Status code: {response.status_code}[/bold green]"
                )
                try:
                    if response.headers.get("Content-Type", "").startswith(
                        ...
                        console.print(
                            f"[dim]Response body: {json.dumps(response.json(), indent=2)}[/dim]"
                        )
                    else:
                        console.print(
                            f"[dim]Response: {response.text[:200]}{'...' if len(response.text) > 200 else ''}[/dim]"
                        )
                except:
                    pass
                return True
    ...
    console.print("[dim]Checking WebSocket connection...[/dim]")
    try:
        import socketio
        ...
        def connect():
            ...
            connected = True
            console.print(
                "[bold green]✓ WebSocket connected successfully![/bold green]"
            )
            sio.disconnect()

        @sio.event
        def connect_error(data):
            console.print(f"[bold red]x WebSocket connection error: {data}[/bold red]")
        ...
        console.print(f"[bold red]x WebSocket connection failed: {str(e)}[/bold red]")

    # If we get here, all connection attempts failed
    console.print(
        Panel(
            "[bold red]x Server appears to be offline or not responding![/bold red]\n\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Check if the Zirak API server is running"
    )
    parser.add_argument(
