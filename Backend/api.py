...
)  # Configure CORS with proper origins in production
socketio = SocketIO(
...
class WebSocketHandler(logging.Handler):
    ...
    def emit(self, record):
        try:
            ...
            if any(
                keyword in msg
                for keyword in ["agent", "Agent", "tool", "conversation", "workflow"]
            ):
...
def chat():
    ...
    try:
        ...
        logger.info(
            f"Received chat request with message: {user_input} for conversation {conversation_id}"
        )
        ...
        group_chat.context = {"user_input": user_input, "workflow_id": conversation_id}
        ...
        logger.info(
            f"Conversation workflow {conversation_id} completed with success={success}"
        )
    ...
    try:
        ...
        for conv_id, messages in conversations.items():
            ...
            if conv_id in active_conversations:
                status = (
                    "active"
                    if not active_conversations[conv_id].get("completed", False)
                    else "completed"
                )
            ...
            conversation_list.append(
                {
                    "id": conv_id,
                    "title": title,
                    "last_activity": active_conversations.get(conv_id, {}).get(
                        "last_activity", ""
                    ),
        ...
        return jsonify(
            {
                ...
                "workflow_steps": workflow_steps.get(conversation_id, []),
                "status": "active"
                if not active_conversations.get(conversation_id, {}).get(
                    "completed", True
                )
                else "completed",
            }
...
def workflow_status():
    ...
    if conversation_id in workflow_steps:
        return jsonify(
            {
                "conversation_id": conversation_id,
                "events": workflow_steps[conversation_id],
            }
        )
