        ...
        if "processing message from" in message.lower():
            ...
            if len(parts) > 1:
                ...
                conversation_sequence.append(
                    f"[bold blue]{agent} ← {sender}[/bold blue]"
                )
        elif "to chat_manager" in message.lower():
            conversation_sequence.append(
                f"[bold magenta]{agent} → chat_manager[/bold magenta]"
            )
        elif "generated reply" in message.lower():
            conversation_sequence.append(
                f"[bold green]{agent} generated reply[/bold green]"
            )
...
def poll_for_updates(max_time=60, inactivity_timeout=10, poll_interval=0.5):
    ...
    console.print(
        f"[bold cyan]Waiting for agent communications for up to {max_time} seconds...[/bold cyan]"
    )
    console.print(
    ...
    with Progress(
        SpinnerColumn(),
        TextColumn(
            "[bold blue]Collecting data...[/bold blue] (Press Ctrl+C to stop manually)"
        ),
        console=console,
    ) as progress:
...
def main():
    parser = argparse.ArgumentParser(
        description="Test the Zirak API with a simple query"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=60,
        help="Maximum time to wait for updates (seconds)",
    )
    parser.add_argument(
        "--inactivity-timeout",
        type=int,
        default=5,
        help="Stop collecting after this many seconds of inactivity",
    )
    parser.add_argument(
        "--no-wait",
        action="store_true",
        help="Skip waiting for updates after initial response",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    try:
        ...
        try:
            sio.connect(
                socket_url, wait_timeout=10, transports=["websocket", "polling"]
            )
        ...
        if response:
            ...
            console.print(
                f"[bold green]Full conversation data saved to conversation_{conversation_id}.json[/bold green]"
            )
