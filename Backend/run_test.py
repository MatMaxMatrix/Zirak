...
def check_server_running(url):
    ...
    try:
            ...
            if choice == 'y' or choice == 'yes':
                try:
                    ...
                    if sys.platform == 'darwin':  # macOS
                        subprocess.Popen(["osascript", "-e", f'tell app "Terminal" to do script "cd {os.getcwd()} && python api.py"'])
                    elif sys.platform == 'win32':  # Windows
                        subprocess.Popen(["start", "cmd", "/k", "python", "api.py"], shell=True)
                    else:  # Linux and others
                        subprocess.Popen(["x-terminal-emulator", "-e", f"cd {os.getcwd()} && python api.py"])
                    ...
                    with Progress(
                        SpinnerColumn(),
                        TextColumn("[bold blue]Waiting for server to start...[/bold blue]"),
                        console=console
                    ) as progress:
        ...
        console.print(f"[bold red]× Error checking server: {str(e)}[/bold red]")
        return False
...
def main():
    ...
    parser.add_argument("--timeout", type=int, help="How long to collect data (seconds)")
    parser.add_argument("--visualize", action="store_true", help="Use the visualizer script instead of the basic test")
    parser.add_argument("--no-viz", action="store_true", help="Skip generating HTML visualization (only with --visualize)")
    ...
    if not args.skip_checks:
        if not check_dependencies():
            console.print("[bold red]× Cannot proceed due to missing dependencies.[/bold red]")
            return
        
        if not check_server_running(args.url):
            console.print("[bold red]× Cannot proceed as server is not running.[/bold red]")
            return
