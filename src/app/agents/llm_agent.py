...
class LLM_Agent(ConversableAgent):
    def __init__(self, mcp_server_url: str = "http://localhost:3002/sse"):
        ...
        # MCP connection is established by agent_manager.py via the
        # persistent async event loop (async_runner.run_async).  Do NOT
        # attempt an inline connection here — it would run in a different
        # event loop and cause anyio cancel-scope conflicts.
        self.console.print(
            "[yellow]MCP connection deferred to agent_manager initialisation.[/yellow]"
        )
    ...
    async def connect_to_mcp_server(self) -> bool:
        """
        Wait for the MCPManager worker to be ready, then fetch the tool list.

        The actual SSE connection is owned by MCPManager (mcp_manager.py) and
        lives in the persistent event loop for the entire app lifetime.  This
        method just waits until that connection is established and (re)populates
        self.available_mcp_tools.
        """
        from .mcp_manager import get_mcp_manager

        manager = get_mcp_manager(self.mcp_server_url)
        ready = await manager.wait_ready(timeout=30.0)
        if not ready:
            logger.error("MCPManager not ready after 30 s.")
            self._mcp_connected = False
            return False

        # If tools are already loaded, skip re-fetch
        if self._mcp_connected and self.available_mcp_tools:
        ...
        try:
            ...
            self.console.print(
                f"[green]Connected to MCP server via MCPManager. "
                f"Found {len(self.available_mcp_tools)} tools.[/green]"
            )
            logger.info(
            ...
            logger.error(f"Failed to fetch MCP tools: {exc}", exc_info=True)
            self._mcp_connected = False
            return False

    async def _fetch_and_process_mcp_tools(self):
        """Fetches and processes the list of available tools via MCPManager."""
        from .mcp_manager import get_mcp_manager
        manager = get_mcp_manager(self.mcp_server_url)

        try:
            logger.info("Fetching available tools via MCPManager...")
    ...
    async def refresh_tools(self) -> bool:
        ...
        """
        self.console.print("[cyan]Refreshing tools via MCPManager...[/cyan]")
        # Clear local tool cache; connect_to_mcp_server will re-fetch from manager
        self._mcp_connected = False
        self.available_mcp_tools = []
    ...
    async def _execute_tool(self, tool_use):
        ...
        # Ensure tools are loaded (manager handles the actual connection)
        if not self._mcp_connected or not self.available_mcp_tools:
            self.console.print(
                "[yellow]MCP tools not yet loaded. Connecting...[/yellow]"
            )
            success = await self.connect_to_mcp_server()
            if not success:
                return f"Error: Failed to connect to MCP server to execute tool '{tool_name}'"

        # Execute the tool via MCPManager (which owns the persistent session)
        from .mcp_manager import get_mcp_manager
        mcp_mgr = get_mcp_manager(self.mcp_server_url)

        result = None
        raw_mcp_result = None
        try:
            self.console.print(
                f"[cyan]Executing tool '{tool_name}' via MCPManager...[/cyan]"
            )
            with Live(
                Spinner("dots", text="[cyan]Tool execution in progress...[/cyan]"),
                refresh_per_second=10,
            ) as live:
                raw_mcp_result = await mcp_mgr.call_tool(
...
"""
    ...
    async def cleanup(self):
        ...
        # MCP connection is now owned by MCPManager — no cleanup needed here.
        # Just reset the local tool cache so it gets re-fetched on next use.
        self._mcp_connected = False
        self.available_mcp_tools = []
    ...
    def main(self, *args, **kwargs):
        """
        Synchronous entry point required by AutoGen's initiate_chat().

        Runs the async implementation in a dedicated OS thread so it gets its
        own event loop, avoiding eventlet/asyncio conflicts.
        """
        import concurrent.futures

        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            return pool.submit(asyncio.run, self._main_async(*args, **kwargs)).result()

    async def _main_async(self, *args, **kwargs):
        """
        Async implementation of the agent's main reply function.
        ...
        try:
            ...
            if args or kwargs:
                ...
                if messages:
                    ...
                    if steps_map:
                        step_keys = sorted(
                            k for k in steps_map.keys() if k.startswith("step")
                        )
                        total = len(step_keys)
                        step_response = "No specific step response."

                        while self.current_step_index < total:
                            current_step_key = step_keys[self.current_step_index]
                            current_step = steps_map[current_step_key]
                            ...
                            if isinstance(current_step, dict) and "description" in current_step:
                                ...
                                prompt = f"Execute step: {step_description}\n\nDetails: {step_details}"
                            else:
                                step_description = str(current_step)
                                prompt = f"Execute step: {current_step}"

                            self.console.print(
                                f"[bold cyan]Step {step_num}/{total}: {step_description[:80]}[/bold cyan]"
                            )
                            if log:
                                log(wid, "LLM_Agent", f"⚙️ **Step {step_num}/{total}:** {step_description}")

                            self.current_step_index += 1
                            step_response = await self.chat(prompt)

                            # Stream the step result to the UI
                            if log and step_response:
                                log(wid, "LLM_Agent", f"✅ **Step {step_num} result:**\n{step_response}")

                        self.console.print("[bold yellow]All steps completed.[/bold yellow]")
                        if log:
                            log(wid, "LLM_Agent", f"✅ All {total} steps completed.")
                        final_response_content = step_response

                    # No step plan — process the message content directly
                    elif content:
                        if log:
                            log(wid, "LLM_Agent", "⚙️ Processing your request…")
                        final_response_content = await self.chat(content)
                        if log and final_response_content:
                            log(wid, "LLM_Agent", final_response_content)
            ...
            if (
                ...
                if user_input.lower() == "quit":
                    ...
                    return await self._main_async(*args, **kwargs)
                else:
            ...
            # Emit final summary to the UI
            ctx = getattr(self, "context", {})
            log = ctx.get("log_agent_message")
            wid = ctx.get("workflow_id", "")
            if log and final_summary and final_summary != "No specific actions were recorded to summarize for this task.":
                log(wid, "LLM_Agent", f"**Summary:**\n{final_summary}")

            # Append TERMINATE so the GroupChatManager stops the conversation
            return True, {"role": "assistant", "content": f"{final_summary}\n\nTERMINATE"}
