...
async def conversation_workflow(group_chat):
    ...
    try:
        ...
        streaming_console.print("[bold cyan]Group chat manager reset successfully[/bold cyan]")
        ...
        streaming_console.print(f"[bold cyan]Setting up context with user input: {user_input}[/bold cyan]")
        
        # Add instrumentation to all agents to capture their interactions
        for agent in group_chat.agents:
            agent.context = group_chat.context
            streaming_console.print(f"[bold cyan]Context set for agent: {agent.name}[/bold cyan]")
        ...
        if "clarifications" not in group_chat.context:
            group_chat.context["clarifications"] = "Clarifications: Nothing to clarify yet."
        ...
        try:
            ...
            try:
                ...
                streaming_console.print("[bold cyan]Using synchronous initiate_chat method[/bold cyan]")
                try:
                    await asyncio.wait_for(initiating_agent.initiate_chat(
                        group_chat_manager, 
                    ...
                    streaming_console.print(f"[bold yellow]Sync version failed, trying async: {str(sync_e)}[/bold yellow]")
                    if hasattr(initiating_agent, 'a_initiate_chat') and inspect.iscoroutinefunction(initiating_agent.a_initiate_chat):
                        # If we have an async version, use it
                        streaming_console.print("[bold cyan]Using async initiate_chat method[/bold cyan]")
                        ...
                        streaming_console.print("[bold yellow]Both methods failed, using manual conversation start[/bold yellow]")
                        ...
                        chat_result = {"role": "assistant", "content": "Conversation started manually"}
                        
                streaming_console.print(f"[bold cyan]Initiate chat returned: {chat_result}[/bold cyan]")
            except Exception as e:
            ...
            while not conversation_completed and (time.time() - start_time_conv) < timeout:
                # Check for conversation completion using multiple methods
                
                # Method 1: Check _terminated flag (most reliable)
                if hasattr(group_chat_manager, '_terminated') and group_chat_manager._terminated:
                    conversation_completed = True
                    streaming_console.print("[bold green]Conversation completed (_terminated flag)[/bold green]")
                    break
                
                # Method 2: Check is_terminated method
                if hasattr(group_chat_manager, 'is_terminated') and callable(group_chat_manager.is_terminated):
                    try:
                        if group_chat_manager.is_terminated():
                            conversation_completed = True
                            streaming_console.print("[bold green]Conversation completed (terminated flag)[/bold green]")
                            break
                    except Exception as e:
                        streaming_console.print(f"[yellow]Error checking termination: {str(e)}[/yellow]")
                ...
                try:
                    ...
                    if messages and any("TERMINATE" in str(msg.get('content', '')) for msg in messages):
                        conversation_completed = True
                        streaming_console.print("[bold green]Conversation completed (TERMINATE found)[/bold green]")
                        break
                    ...
                    if len(messages) > 3:
                        ...
                        if len(last_two_messages) >= 2:
                            ...
                            if last_speaker == previous_speaker:
                                streaming_console.print("[bold yellow]Warning: Same speaker spoke twice in a row, potential conversation loop[/bold yellow]")
                                
                        # Check for substantial final message
                        last_message = messages[-1]
                        if last_message.get('content', '') and len(last_message.get('content', '').split()) > 20:
                            streaming_console.print("[bold yellow]Substantial message found, may be complete[/bold yellow]")
                            
                            # If no activity for 5+ seconds after substantial message
                            if time.time() - start_time_conv > 5:
                                streaming_console.print("[bold green]No activity for 5+ seconds after substantial message, considering complete[/bold green]")
                                conversation_completed = True
                                break
                except Exception as e:
                    streaming_console.print(f"[yellow]Error checking message history: {str(e)}[/yellow]")
                ...
                if elapsed % 5 < 0.2:  # Log roughly every 5 seconds
                    streaming_console.print(f"[dim]Waiting for conversation to complete... ({elapsed:.1f}s elapsed)[/dim]")
            
            # Handle timeout
            if not conversation_completed and (time.time() - start_time_conv) >= timeout:
                streaming_console.print("[bold yellow]Conversation timed out[/bold yellow]")
            ...
            final_response = ""
            if hasattr(group_chat, 'messages') and group_chat.messages and len(group_chat.messages) > 0:
            ...
            streaming_console.print(f"[bold red]Error during conversation: {str(e)}[/bold red]")
        ...
        streaming_console.print(f"[bold cyan]Total conversation time: {total_time:.2f} seconds[/bold cyan]")
        ...
        for agent in group_chat.agents:
            if hasattr(agent, 'total_tokens_used'):
                ...
                streaming_console.print(f"[bold cyan]Agent {agent.name} used {agent.total_tokens_used} tokens[/bold cyan]")
        
        # Log detailed information
        logger.info(f"Workflow {workflow_id} completed | Success: {success} | Time: {total_time:.2f}s | Token Usage: {json.dumps(token_usage)}")
