...
def display_questions_and_answers():
    ...
    for i, qa in enumerate(questions_and_answers, 1):
        ...
        console.print(f"[bold {agent_color}]Q{i} from {agent} at {time}:[/bold {agent_color}]")
    ...
    for agent_name in agent_communications.keys():
        if agent_name not in nodes_set:
            ...
            nodes.append({
                ...
                'label': agent_name,
                'color': color,
                'size': agent_message_counts[agent_name] * 5  # Size based on message count
            })
...
def main():
    parser = argparse.ArgumentParser(description="Test the Zirak API with a simple query and visualize agent interactions")
    ...
    parser.add_argument("--timeout", type=int, default=30, help="How long to collect data (seconds)")
    parser.add_argument("--no-viz", action="store_true", help="Skip generating HTML visualization")
    parser.add_argument("--no-socket", action="store_true", help="Skip WebSocket connection and only use HTTP")
    ...
    try:
        ...
        if response:
            ...
            if not args.no_socket and sio.connected:
                console.print(f"[bold cyan]Collecting agent communications for {args.timeout} seconds...[/bold cyan]")
                with Progress(
                    SpinnerColumn(),
                ...
                console.print("[yellow]Skipping data collection as WebSocket is not connected.[/yellow]")
            ...
            if not args.no_viz:
                ...
                if html_file:
                    console.print(f"[bold green]HTML visualization saved to: {html_file}[/bold green]")
                    console.print(f"[bold yellow]Open this file in a web browser to see the interactive visualization[/bold yellow]")
            ...
            console.print(f"[bold green]Full conversation data saved to {json_file}[/bold green]")
        else:
