...
class TestWebScraperTool:
    ...
    def test_execute_handles_request_error(self, requests_mock):
        import requests
        from app.agents.tools.web_scraper_tool import WebScraperTool
...
class TestFileCreatorTool:
    ...
    def test_creates_file(self, tmp_path):
        ...
        result = tool.execute(files={"path": target, "content": "Hello, world!"})

        data = json.loads(result)
        assert data["created_files"] == 1
    ...
    def test_creates_nested_dirs(self, tmp_path):
        ...
        result = tool.execute(files={"path": target, "content": "# empty"})
    ...
    def test_creates_multiple_files(self, tmp_path):
        ...
        result = tool.execute(files=[
            {"path": str(tmp_path / "a.txt"), "content": "aaa"},
            {"path": str(tmp_path / "b.txt"), "content": "bbb"},
        ])

        data = json.loads(result)
        assert data["created_files"] == 2
...
class TestFileContentReaderTool:
    ...
    def test_reads_existing_file(self, tmp_path):
        ...
        result = tool.execute(file_paths=[str(f)])

        data = json.loads(result)
        assert "test content" in list(data.values())[0]

    def test_handles_missing_file(self, tmp_path):
        from app.agents.tools.file_content_reader_tool import FileContentReaderTool

        tool = FileContentReaderTool()
        result = tool.execute(file_paths=[str(tmp_path / "nonexistent.txt")])
...
class TestFileEditTool:
    ...
    def test_full_replace_content(self, tmp_path):
        from app.agents.tools.file_edit_tool import FileEditTool
    ...
    def test_pattern_replace(self, tmp_path):
        from app.agents.tools.file_edit_tool import FileEditTool
...
class TestCreateFoldersTool:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        result = tool.execute(folder_paths=[target])
    ...
    def test_idempotent_on_existing(self, tmp_path):
        ...
        result = tool.execute(folder_paths=[str(tmp_path / "exists")])
